...
pref-pdf-native-overlay-reuse =
    .label = 复用 Zotero 10 引用分析结果
pref-pdf-native-overlay-reuse-desc = 在后台复用 Zotero 10 已完成的引用分析，使引用悬停与查询保持流畅。仅对经过源码审计的 Zotero 10.0 构建启用，重启 Zotero 后生效；其他文档继续使用原有匹配路径。
