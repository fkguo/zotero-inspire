...
menuitem-citation-graph-merge = 关系图…
...
connections-graph-title = 关系图
references-panel-citation-graph-button = 关系图
references-panel-citation-graph-tooltip = 显示关系图：引用关系图与学术谱系（左键打开，右键展开）
...
# Academic genealogy
academic-tree-title = 学术谱系
academic-tree-back = ← 后退
academic-tree-forward = 前进 →
academic-tree-maximize = 最大化／还原窗口
academic-tree-search-placeholder = 作者姓名或 INSPIRE BAI
academic-tree-search = 搜索作者
academic-tree-searching = 正在搜索作者…
academic-tree-choose = 请选择作者以查看学术谱系。
academic-tree-no-authors = 未找到作者，请换用其他姓名或 BAI。
academic-tree-search-error = 作者搜索失败，请重试。
academic-tree-select = 选择作者
academic-tree-up = 上溯代数
academic-tree-down = 下溯代数
academic-tree-depth-limit = 已达到 { $count } 代上限；可点击此人的姓名，以其为中心继续追溯。
academic-tree-degree = 培养类型
academic-tree-all = 全部关系
academic-tree-phd = 博士
academic-tree-master = 硕士
academic-tree-bachelor = 本科
academic-tree-other = Other（其他）
academic-tree-diploma = Diploma
academic-tree-habilitation = Habilitation
academic-tree-laurea = Laurea
academic-tree-specified = 已注明学位／资格
academic-tree-unknown = 未注明
academic-tree-refresh = 刷新 INSPIRE
academic-tree-refresh-hint = 从 INSPIRE 重新获取当前谱系（包括已展开分支）的数据，并更新缓存。
academic-tree-retry = 重试
academic-tree-continue = 继续加载
academic-tree-refreshing = 正在从 INSPIRE 刷新，原谱系图保持显示…
academic-tree-refresh-error = { $count } 个请求失败，原谱系图已保留；可重试完成刷新。
academic-tree-profile-error = { $count } 位作者的资料加载失败，可重试补全。
academic-tree-stop = 停止
academic-tree-more = 加载更多
academic-tree-fit = 适应全图
academic-tree-fit-page = 页面内排布
academic-tree-fit-page-hint = 按窗口宽度将过长的一代分成多排，不缩小姓名。从左到右，再读同一代的下一排。滚轮或拖动可纵向浏览，Ctrl/⌘＋滚轮缩放；关闭可恢复树形排布。
academic-tree-generation = 第 { $generation } 代
academic-tree-generation-row = 第 { $row } / { $rows } 排
academic-tree-center = 定位中心作者
academic-tree-zoom-in = 放大
academic-tree-zoom-out = 缩小
academic-tree-loading = 正在逐层加载…
academic-tree-load-error = 无法加载作者，请重试或选择其他作者。
academic-tree-partial-error = { $count } 个分支加载失败，可重试。
academic-tree-limit = 展开已暂停（节点预算：{ $count }），可加载更多或选取分支查看。
academic-tree-count = { $count } 人 · { $links } 条关系
academic-tree-unlinked = INSPIRE 未关联此姓名的作者记录，无法自动继续展开。
academic-tree-no-advisors = INSPIRE 中未找到符合当前筛选的导师记录。
academic-tree-no-students = INSPIRE 中未找到符合当前筛选的学生记录。
academic-tree-node-error = 此分支加载失败，可点击展开或重试。
academic-tree-expand-up = 展开一代导师
academic-tree-expand-down = 展开一代学生
academic-tree-reroot = 设为中心作者
academic-tree-inspire = INSPIRE 作者页面
academic-tree-papers = 查看论文
academic-tree-stopped = 加载已停止，已加载的师承关系仍保留。
academic-tree-canvas-help = 点击姓名追溯其学术谱系，同时在侧边栏显示作者页面；悬浮姓名显示作者预览卡。点击卡片空白处选择分支操作。拖动平移，滚轮缩放。页面内排布模式下，滚轮纵向浏览，Ctrl/⌘＋滚轮缩放；方向键、+/− 和 0 也可操作。
...
academic-tree-find-empty = 已加载的谱系中没有匹配人物。
academic-tree-locate = 定位（不更换谱系中心）
academic-tree-path-from = 路径起点人物…
academic-tree-path-to = 路径终点人物…
academic-tree-show-path = 显示师承路径
academic-tree-clear-path = 清除路径
academic-tree-path-empty = 请选择已加载谱系中相互连通的两个人物。未找到路径。
academic-tree-expand-ancestors = 展开所有已显示祖先的学生
academic-tree-collapse-ancestors = 收起新增的祖先学生分支
academic-tree-collapse-ancestors-hint = 隐藏本次展开新增的学生分支，保留原有谱系和中心人物的祖先链。再次展开可恢复已加载分支。
academic-tree-expand-ancestors-hint = 对所有已显示代数的祖先，分别加载其直接学生。每人只向下一代，本次新出现的人物不继续递归展开。
academic-tree-collapse = 收起所选人物的学生分支
academic-tree-restore-branch = 恢复所选人物的学生分支
academic-tree-restore-branches = 恢复所有已收起分支
academic-tree-export = 导出
academic-tree-export-scope = 导出范围
academic-tree-export-view = 当前视图
academic-tree-export-full = 全部已加载谱系
academic-tree-export-hint = SVG/PNG 导出所选范围的完整排布，当前视图包含窗口外仍处于显示状态的人物；JSON/CSV 导出所选范围内的人物与关系。完整谱系包含隐藏分支，不额外获取数据。大型谱系建议使用 SVG 保留细节。
academic-tree-export-success = 学术谱系已导出。
academic-tree-export-error = 导出失败，请重试。
academic-tree-visible-count = 显示 { $count } 人
