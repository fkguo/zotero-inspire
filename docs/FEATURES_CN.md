...
- **关系类型**：保留共同导师及同一对人物的多种培养关系，可筛选博士、硕士、本科或其他／未注明。箭头从导师指向学生。
...
- **连线**：主线分叉点可不等距，优先对准可用空隙。跨排连线先减少转折，再缩短距离；单导师箭头可在顶部小幅偏离中心接入，避免末端小折角。共同导师箭头仍分开显示。
...
- **📤 导出▼**：与 Citation Graph 共用弹出菜单和文件保存功能，支持 SVG、PNG、JSON、CSV，可选当前视图或全部已加载谱系。SVG/PNG 导出当前画布区域或完整谱系；JSON/CSV 导出可见或全部已加载人物与关系。JSON 同时记录 ID、筛选、收起与路径状态、视口；CSV 区分人物行与关系行。完整导出包含隐藏分支，不额外联网。图像采用浅色主题；PNG 限制单边 8192 像素、约 1600 万像素，大型谱系推荐 SVG。
