...
| **缓存**              | `Download references cache` _(1.1.3+)_ | 预先将所选条目的 INSPIRE References 写入本地缓存，并显示进度及成功/失败统计 |
| **复制** _(1.1.4+)_ | `Copy BibTeX`                            | 从 INSPIRE 获取并复制 BibTeX                                                |
...
References Panel 支持完整的键盘导航，无需鼠标即可高效浏览：
...
| `Home`                | 跳转到列表首项                                                |
| `End`                 | 跳转到列表末项                                                |
| `Enter`               | 打开 PDF（如有）、选中库中条目、或触发导入                    |
| `Space` / `l`       | 切换关联状态（链接/取消链接相关条目）                         |
| `Tab` / `Shift+Tab` | 切换标签页（References → Cited By → Entry Cited → Search） |
| `Ctrl/Cmd+C`          | 复制当前焦点条目的 BibTeX 到剪贴板                            |
| `Escape`              | 清除焦点选中状态                                              |

**提示**：

- Vim 用户可使用 `j`/`k` 作为方向键的替代
- 焦点条目以蓝色左边框和淡蓝背景高亮显示
- 键盘导航与过滤器配合使用——在过滤结果中导航
...
在 PDF 中选中引用后，悬浮在弹出的查找按钮上可预览条目详情：

- **预览卡片**：显示标题、作者、摘要、出版信息等
- **多义引用提示**：当作者-年份引用存在多个候选时，显示提示"仅作者-年份匹配；点击选择"
...
- **搜索文本缓存**: 避免重复计算搜索字符串
- **缓存统计监控** _(v2.1.0)_: LRUCache 支持命中/未命中统计，可用于性能分析
...
_本文档对应插件版本：2.1.1（最后更新：2025-12-14）_
