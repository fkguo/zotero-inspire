...
- **默认过滤**：默认会避开 PDG 的 _Review of Particle Physics_（过于通用，容易破坏相关性），并可在偏好设置中选择是否排除综述类文章。
...
在经过源码审计的 Zotero 10.0 构建上，插件会在后台复用 Zotero 已经完成的引用分析，不再为引用悬停重复解析整篇 PDF。悬停和点击不会等待后台结果：结果尚未就绪、版本不受支持，或 PDF 超过 Zotero 原生生产器的 100 页边界时，都会立即使用原有匹配链。多千页的 _Review of Particle Physics_ 因而不会触发私有 overlay 扫描。

该功能默认启用，可在“PDF 引用检测设置”中关闭；设置在重启 Zotero 后生效。
...
- **使用 INSPIRE 引用键**：将 INSPIRE 的 texkey 写入 Citation Key 字段（Zotero 8+）或 Extra 字段（Zotero 7）
...
- **复用 Zotero 10 引用分析结果**：后台复用已完成的 Zotero 10.0 结果；关闭或开启后需重启 Zotero
...
- 自动设置 INSPIRE Citation Key（Zotero 8+ 写入原生字段，Zotero 7 写入 Extra 字段）
- BBT 可读取任一位置的 Citation Key
