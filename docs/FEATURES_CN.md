...
显示层级根据师生关系排列，并兼顾共同导师关系，使相关导师靠近其学生。

通过作者预览卡或 Author Papers 页面中的 **学术谱系** 按钮打开，也可在 关系图（Connections Graph）浮窗顶部切换到 **Academic Tree** 并搜索作者。
...
- **视图菜单**：可隐藏补充的共同导师，同时保留中心人物自己的导师和已展开的祖先学生分支。在已加载谱系中按姓名、INSPIRE ID 或单位查找并定位，不更换中心、不请求网络。选择两个人物可高亮最短师承连接路径并自动缩放到该路径；每段箭头保持原方向，也可展示经共同导师连接的路径。
- **展开菜单**：一次展开当前可见祖先的一代学生，遵守既有代数、人数和请求上限。可收起所选人物的学生分支、恢复单个分支或恢复全部分支，无需重复请求。共享学生若还有其他可见连接则保留；中心人物的祖先链不允许收起。定位隐藏人物或显示隐藏路径会恢复所需显示。
- **导出菜单**：支持 SVG、PNG、JSON、CSV，可选当前视图或全部已加载谱系。SVG/PNG 导出当前画布区域或完整谱系；JSON/CSV 导出可见或全部已加载人物与关系。JSON 同时记录 ID、筛选、收起与路径状态、视口；CSV 区分人物行与关系行。完整导出包含隐藏分支，不额外联网。图像采用浅色主题；PNG 限制单边 8192 像素、约 1600 万像素，大型谱系推荐 SVG。
