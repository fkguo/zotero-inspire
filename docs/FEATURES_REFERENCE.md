...
- Set ancestor and descendant depths independently from **0 to 8**, with **2 generations in each direction** by default. Zero hides that direction. Branch expansion also stops at eight generations from the current root; click a boundary author's name to continue from a new center.
...
- **Co-advisors toggle**: show or hide supplemental co-advisors while keeping the center’s own mentors and expanded ancestor branches. **Find / path**: search loaded names, INSPIRE IDs and affiliations to locate a person without rerooting. Highlight the shortest relationship path between two loaded people, preserving arrow directions; the canvas fits that path.
- **Branches menu**: expand one student generation of the currently displayed ancestors. Collapse a selected student branch or restore one/all collapsed branches without fetching data. Shared descendants remain when connected by another visible route; the center’s ancestral chain is protected. Locating a hidden person or showing a hidden path restores visibility.
- **📤 Export▼** uses the same popup and native file-saving code as Citation Graph: SVG/PNG export the current viewport or full loaded tree; JSON/CSV export visible or all loaded people and relationships. JSON includes IDs, filters, collapse/path state and viewport. CSV distinguishes person and relationship rows. Full-tree export includes hidden branches without network requests. Images use a light theme; PNG is bounded to 8192 pixels per side and 16 megapixels, while SVG retains full vector detail.
