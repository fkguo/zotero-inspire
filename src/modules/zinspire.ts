...
`);
  ...
  private getOrCreateLabelMatcher(
    ...
    matcher = new LabelMatcher(entries, attachmentItemID);
...
toolbarbutton.zinspire-refresh.section-custom-button.zinspire-section-button-loading img {
  ...
  private loadSearchHistory() {
    this.searchHistory = literatureSearchHistory.read();
  }
    ...
    link.addEventListener("click", (e) => {
      ...
      if (paper.itemID) {
        ...
        if (item && !item.deleted) {
          Zotero.getActiveZoteroPane()?.selectItem(paper.itemID);
        } else if (paper.recid) {
  ...
  private createFavoritePresentationRow(
    ...
    link.addEventListener("click", (e) => {
      ...
      if (presentation.itemID) {
        ...
        if (item && !item.deleted) {
          Zotero.getActiveZoteroPane()?.selectItem(presentation.itemID);
